...
export const DisplacementSphere = props => {
  ...
  useEffect(() => {
    ...
    startTransition(() => {
      geometry.current = new IcosahedronGeometry(32, 1);
      sphere.current = new Mesh(geometry.current, material.current);
      
      const outerGeometry = new IcosahedronGeometry(34, 1);
      ...
      sphere.current.add(outerSphere);
  ...
  useEffect(() => {
    const dirLight = new DirectionalLight(0x00FF6A, theme === 'light' ? 1.8 : 2.0);
    const ambientLight = new AmbientLight(0x00FF6A, theme === 'light' ? 2.7 : 0.4);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (uniforms.current !== undefined) {
        uniforms.current.time.value = 0.00001 * (Date.now() - start.current);
      }
